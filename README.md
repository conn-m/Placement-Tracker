...
This .NET project ```PlacementTracker``` provides a solution containing separate Data, Test and Web(MVC) projects. When installed and used to create a new project, all references to PlacementTracker will be replaced with the name of your project.
...
1. An implementation of ```PlacementTracker.Data.Services.IUserService``` using EntityFramework to handle data storage/retrieval is provided via ```PlacementTracker.Data.Services.UserServiceDb```.
2. An implementation of ```PlacementTracker.Data.Services.IMailService``` using the .NET Smtp Mail provider to handle email sending is provided via ```PlacementTracker.Data.Services.SmtpMailService```.

Password hashing functionality is added via the ```PlacementTracker.Data.Security.Hasher``` class. This is used in the Data project ```UserServiceDb``` to hash the user password before storing in database.
...
## Install PlacementTracker

To install this solution as a placementtracker (placementtracker name is **termonbase**)

1. Download current version of the placementtracker

    ```$ git clone https://github.com/termon/DotNetPlacementTrackerBase.git```

2. Install the placementtracker so it can be used by ```dotnet new``` command. Use the path (i.e the directory location)to the cloned placementtracker directory without trailing '/'

    Linux/macOS

    ```$ dotnet new install /path/DotNetPlacementTrackerBase```

    Windows

    ```c:> dotnet new install c:\path\DotNetPlacementTrackerBase```

3. Once installed you can create a new project using this placementtracker.

    ```dotnet new termonbase -n SolutionName```

4. To uninstall a placementtracker (no longer can be used with dotnet new ).

    ```dotnet new uninstall /path/DotNetPlacementTrackerBase```
